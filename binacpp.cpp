 ...
 *                          TRADE                           *
 * **********************************************************/
QByteArray Binacpp::trade(QString price, QString amount, QString symbol, QString side, QString type, QString window)
{
    QString head = QString("symbol=%1&side=%2&type=%3&timeInForce=GTC&quantity=%4&price=%5&recvWindow=%6&")
            .arg(symbol)
            .arg(side)
            .arg(type)
            .arg(amount)
...
QByteArray Binacpp::getMyTrades(QString symbol, QString limit)
{
    QString url = "https://www.binance.com/api/v3/myTrades?";
    QString head = QString("symbol=%1&limit=%2&")
            .arg(symbol)
            .arg(limit);
    QByteArray data(head.toUtf8());
    data.append(getTimeStamp());
...
QByteArray Binacpp::getMyAllTrades(QString symbol, QString limit)
{
...
QByteArray Binacpp::getMyAllTrades(QString symbol, QString limit, QString startTime, QString endTime)
{
    QString url = "https://www.binance.com/api/v3/allOrders?";
    QString head = QString("symbol=%1&limit=%2&startTime=%3&endTime=%4&")
            .arg(symbol)
            .arg(limit)
            .arg(startTime)
            .arg(endTime);
...
//    qDebug() << "******START REQUEST*******";
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        parsError(data);
        delete reply;
        return QByteArray("Error Binacpp:getServerData");
    }
//	qDebug() << "*******END REQUEST******";
    QByteArray data = reply->readAll();
    reply->deleteLater();
